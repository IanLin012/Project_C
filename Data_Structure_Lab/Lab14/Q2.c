...
void printAllPath(char*, int, char*, int, int);
...
int main() {
    ...
    while(fgets(input, maxChar, stdin)) {
        switch(input[0]) {
            ...
            case '3':
                if(size) {
                    printAllPath(tree, size, input, 0, 0);
                }
                else {
                    printf("NULL\n");
                }
...
void printAllPath(char *tree, int size, char *path, int i, int layer) {
    if(i == 0) {
        memset(path, 0, maxChar);
    }
    ...
    if(tree[i*2+1] != 0) {
        printAllPath(tree, size, path, i*2+1, layer+1);
    }
    if(tree[i*2+2] != 0) {
        printAllPath(tree, size, path, i*2+2, layer+1);
    }
    path[layer] = 0;
}
