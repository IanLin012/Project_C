...
polyPointer padd(polyPointer a, polyPointer b) { // fn 4
    polyPointer NewPoly = (polyPointer)malloc(sizeof(polyNode));
    NewPoly->link = NULL;
    while (a != NULL || b != NULL) {
        int coef, expo;
 ...
 if (NewPoly->link == NULL) {
        polyPointer ZeroNode = (polyPointer)malloc(sizeof(polyNode));
        ZeroNode->coef = 0;
        ZeroNode->expo = 0;
        ZeroNode->link = NULL;
        NewPoly->link = ZeroNode;
    }
    return NewPoly;
}
...
void PolyMultiOperation(){
    ...
    polyPointer result = (polyPointer)malloc(sizeof(polyNode));
    result->coef = 0;
    result->expo = 0;
    result->link = NULL;
    while (top != -1) {
    ...
    if (temp[top] == '+') {
        top--;
        char alphabet = temp[top];
        for (int i = 0; i < MAX_POLYS; i++) {
            if (alphabet == POLYNames[i]) {
                index = i;
                break;
            }
        }
        ...
        result = padd(result->link, POLYS[index]->link);
    }
    else if (temp[top] == '-') {
        top--;
        char alphabet = temp[top];
        ...
        result = psub(result->link, POLYS[index]->link);
    }
    top--;
    }
    if (c[1] == '=') {
...
int main() {
    ...
    while (scanf("%d", &choice) != EOF) {
        if (choice == 0) {
            ...
            polyPointer ptr_a = POLYS[index]->link;
            index = -1;
            for (int i = 0; i < MAX_POLYS; i++) {    // ?? Name ?????????
                if (c[b] == POLYNames[i]) {
                    index = i;
                    break;
                }
            ...
            if (c[1] == '=') {
                ...
                POLYS[redex] = psub(ptr_a, ptr_b);
                printf("%c=", POLYNames[redex]);
                printPoly(POLYS[redex]->link);
            }
            else {
                polyPointer result = psub(ptr_a, ptr_b);
                printf("%s=", c);
                printPoly(result->link);
                free(result);
            }
