#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>

#define maxNode 1001
#define maxChar 10001

void inputTree(int *tree, int *size) {
    char *input = (char*) malloc(sizeof(char) * maxChar);
    for(int n=1; n<maxNode; n*=2) {
        fgets(input, maxChar, stdin);
        char *temp = strtok(input, " \n");
        for(int i=0; i<n; i++) {
            if(temp[0] == 'e' && temp[1] == 'n' || temp[2] == 'd') {
                *size = n-1; 
                printf("OK\n");
                return;
            }
            if(temp[0]=='9' && temp[1]=='9') {
                tree[n-1+i] = 0;
            }
            else {
                tree[n-1+i] = atoi(temp);
            }
            temp = strtok(NULL, " \n");
        }
    }
}

int checkSum(int *tree, int size, int i, int sum) {
    if(sum == 0) {
        return 1;
    }
    if(sum<0 || i>=size) {
        return 0;
    }
    return(checkSum(tree, size, i*2+1, sum-tree[i]) || checkSum(tree, size, i*2+2, sum-tree[i]));
}

int main() {
    int *tree = (int*) malloc(sizeof(int) * maxNode);
    int size = 0;
    char *input = (char*) malloc(sizeof(char) * 15);
    while(fgets(input, 15, stdin)) {
        switch (input[0]) {
            case '0':
                printf("quit\n");
                return 0;
            case '1':
                inputTree(tree, &size);
                break;
            case '2':
                if (!size) {
                    printf("ERROR\n");
                    break;
                }
                fgets(input, 15, stdin);
                if(checkSum(tree, size, 0, atoi(input)) == 1) {
                    printf("YES\n");
                }
                else {
                    printf("NO\n");
                }
                break;
        }
    }
    inputTree(tree, &size);
    return 0;
}